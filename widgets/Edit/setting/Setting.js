    ...
    return declare([BaseWidgetSetting, _WidgetsInTemplateMixin], {
      ...
      _initCustomSettingRadio: function() {
        if(this.config.editor.honorSettingOfWebmap) {
          this.webmapRadio.set('checked', true);
          ...
          this.customRadio.set('checked', true);
        }
        this._controlCustomSettingRadio();

        this.own(on(this.webmapRadio, 'click', lang.hitch(this, this._onRadioClicke)));
        this.own(on(this.customRadio, 'click', lang.hitch(this, this._onRadioClicke)));
      ...
      _initTooltipDialog: function() {
        ...
        var tooltipDialogContent = html.create("div", {
          'innerHTML': jimuUtils.sanitizeHTML(toolTipText),
      ...
      _getEditableTableInfos: function() {
        ...
        var editableJimuLayerInfoArray = [];
        var tableInfoArray = this._jimuLayerInfos.getTableInfoArray();

        var allRlatedLayerOrTableInfoArray = [];
        array.forEach(this._editableLayerInfos, function(layerInfo) {
          var jimuLayerInfo = this._jimuLayerInfos._findTopLayerInfoById(layerInfo.featureLayer.id);
          if(jimuLayerInfo) {
            editableJimuLayerInfoArray.push(jimuLayerInfo);
          }
        }, this);
        array.forEach(editableJimuLayerInfoArray.concat(tableInfoArray), function(layerInfo) {
        ...
        return all(defs).then(lang.hitch(this, function(result) {
          array.forEach(result, function(relatedTableInfoArray) {
            allRlatedLayerOrTableInfoArray = allRlatedLayerOrTableInfoArray.concat(relatedTableInfoArray);
          }, this);
          array.forEach(tableInfoArray, function(jimuTableInfo) {
            var tableObject = jimuTableInfo.layerObject;
            var capabilities = jimuTableInfo.getCapabilitiesOfWebMap();
            var isEditableInWebMap;
            var isAnRelatedTable = array.some(allRlatedLayerOrTableInfoArray, function(relatedLayerInfo) {
              return jimuTableInfo.id === relatedLayerInfo.id;
            }, this);
      ...
      _resetCustomSettingConfig: function() {
        this.config.editor.honorSettingOfWebmap = this.webmapRadio.get('checked') ? true : false;
      },
      ...
      _controlCustomSettingRadio: function() {
        if(this.webmapRadio.get('checked')){
          html.addClass(this.layersSettingPart, 'disable');
        } else if(this.customRadio.get('checked')){
